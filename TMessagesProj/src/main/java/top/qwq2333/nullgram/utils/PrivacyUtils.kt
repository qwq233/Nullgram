...
object PrivacyUtils {
    ...
    fun postCheckAll(ctx: Context, account: Int) {
        ...
        if (!MessagesController.getMainSettings(account)
                .getBoolean("privacy_warning_skip_phone_number", false)
        ) {
        ...
        if (!MessagesController.getMainSettings(account)
                .getBoolean("privacy_warning_skip_add_by_phone", false)
        ) {
        ...
        if (!MessagesController.getMainSettings(account)
                .getBoolean("privacy_warning_skip_p2p", false)
        ) {
        ...
        if (!MessagesController.getMainSettings(account)
                .getBoolean("privacy_warning_skip_2fa", false)
        ) {
    ...
    private fun postCheckAllow2fa(ctx: Context, account: Int) {
        ...
        ConnectionsManager.getInstance(account)
            .sendRequest(TLRPC.TL_account_getPassword(), { response, _ ->
    ...
    private fun showPrivacyAlert(ctx: Context, account: Int, type: Int) {
        ...
        builder.setPositiveButton(
            LocaleController.getString(
                "ApplySuggestion", R.string.ApplySuggestion
            )
        ) { _, _ ->

            ConnectionsManager.getInstance(account)
                .sendRequest(TLRPC.TL_account_setPrivacy().apply {
        ...
        runCatching {
            ...
            (builder.show().getButton(DialogInterface.BUTTON_NEUTRAL) as TextView?)?.setTextColor(
                Theme.getColor(Theme.key_dialogTextRed2)
            )
    ...
    private fun show2faAlert(ctx: Context, account: Int, password: TLRPC.TL_account_password) {
        ...
        builder.setNeutralButton(
            ...
            MessagesController.getMainSettings(account).edit()
                .putBoolean("privacy_warning_skip_2fa", true).apply()
